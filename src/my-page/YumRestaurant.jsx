...
import {fetchGetMyRestaurantList, fetchGetYumRestaurantList} from "../service/RestaurantService.js";
...
const YumRestaurant = () => {
    ...
    const [yumRestaurantList, setYumRestaurantList] = useState([])

    const getYumRestaurantList = async () => {
        const {isError, data} = await fetchGetYumRestaurantList()
        if (isError) {
            alert(data.errorMessage)
            return
        }
        console.log(data)
        setYumRestaurantList(data)
    }

    useEffect(() => {
        getYumRestaurantList()
    }, [])

    return (
        <div className={"flex flex-col gap-y-5 h-[65vh] overflow-scroll"}>
            {yumRestaurantList?.map((restaurant) => (
